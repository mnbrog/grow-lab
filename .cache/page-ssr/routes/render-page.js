...
const getSliceId=(sliceName,sliceProps)=>{if(!Object.keys(sliceProps).length){return sliceName;}const propsString=createContentDigest(sliceProps);return`${sliceName}-${propsString}`;};const ServerSlice=({sliceName,allowEmpty,children,...sliceProps})=>{const slicesMap=(0,external_C_Users_mnbro_grow_lab_node_modules_react_index_js_.useContext)(context.SlicesMapContext);const slicesProps=(0,external_C_Users_mnbro_grow_lab_node_modules_react_index_js_.useContext)(context.SlicesPropsContext);const concreteSliceName=slicesMap[sliceName];if(!concreteSliceName){if(allowEmpty){return null;}else{throw new Error(`Slice "${concreteSliceName}" for "${sliceName}" slot not found`);}}const sliceId=getSliceId(concreteSliceName,sliceProps);// set props on context object for static-entry to return
let sliceUsage=slicesProps[sliceId];if(!sliceUsage){slicesProps[sliceId]=sliceUsage={props:sliceProps,sliceName:concreteSliceName,hasChildren:!!children};}else{if(children){sliceUsage.hasChildren=true;}}return/*#__PURE__*/external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().createElement(server_slice_renderer.ServerSliceRenderer,{sliceId:sliceId},children);};
;// ./.cache/slice/inline-slice.js
const InlineSlice=({sliceName,allowEmpty,children,...sliceProps})=>{const slicesMap=(0,external_C_Users_mnbro_grow_lab_node_modules_react_index_js_.useContext)(context.SlicesMapContext);const slicesResultsMap=(0,external_C_Users_mnbro_grow_lab_node_modules_react_index_js_.useContext)(context.SlicesResultsContext);const concreteSliceName=slicesMap[sliceName];const slice=slicesResultsMap.get(concreteSliceName);if(!slice){if(allowEmpty){return null;}else{throw new Error(`Slice "${concreteSliceName}" for "${sliceName}" slot not found`);}}return/*#__PURE__*/external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().createElement(slice.component,Object.assign({sliceContext:slice.sliceContext,data:slice.data},sliceProps),children);};
;// ./.cache/slice.js
"use client";function Slice(props){if(true){// we use sliceName internally, so remap alias to sliceName
const internalProps={...props,sliceName:props.alias};delete internalProps.alias;delete internalProps.__renderedByLocation;const slicesContext=(0,external_C_Users_mnbro_grow_lab_node_modules_react_index_js_.useContext)(context.SlicesContext);// validate props
const propErrors=validateSliceProps(props);if(Object.keys(propErrors).length){throw new SlicePropsError(slicesContext.renderEnvironment===`browser`,internalProps.sliceName,propErrors,props.__renderedByLocation);}if(slicesContext.renderEnvironment===`server`){return/*#__PURE__*/external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().createElement(ServerSlice,internalProps);}else if(slicesContext.renderEnvironment===`browser`){// in the browser, we'll just render the component as is
return/*#__PURE__*/external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().createElement(InlineSlice,internalProps);}else if(slicesContext.renderEnvironment===`engines`||slicesContext.renderEnvironment===`dev-ssr`){// if we're in SSR, we'll just render the component as is
return/*#__PURE__*/external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().createElement(InlineSlice,internalProps);}else if(slicesContext.renderEnvironment===`slices`){// we are not yet supporting nested slices
...
const fullStack=external_C_Users_mnbro_grow_lab_node_modules_react_index_js_default().__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactDebugCurrentFrame.getCurrentStack();// remove the first line of the stack trace
...
var _react = _interopRequireDefault(__webpack_require__(4504));
...
const React=__webpack_require__(4504);const{grabMatchParams}=__webpack_require__(5870);const{StaticQueryContext}=__webpack_require__(123);const{headExportValidator,filterHeadProps,isElementType,isValidNodeName,warnForInvalidTag}=__webpack_require__(9226);const{ServerLocation,Router}=__webpack_require__(6462);const{renderToString}=__webpack_require__(1552);const{parse}=__webpack_require__(6192);const styleToOjbect=__webpack_require__(9108);function applyHtmlAndBodyAttributesSSR(htmlAndBodyAttributes,{setHtmlAttributes,setBodyAttributes}){if(!htmlAndBodyAttributes)return;const{html,body}=htmlAndBodyAttributes;setHtmlAttributes(html);setBodyAttributes(body);}function getValidHeadNodesAndAttributesSSR(rootNode,htmlAndBodyAttributes={html:{},body:{}}){const seenIds=new Map();const validHeadNodes=[];// Filter out non-element nodes before looping since we don't care about them
...
var React = __webpack_require__(4504);
...
"use strict";
module.exports = require("C:\\Users\\mnbro\\grow-lab\\.cache\\ssr-builtin-trackers\\fs");
...
var React = _interopRequireWildcard(__webpack_require__(4504));
...
var _react = __webpack_require__(4504);
 ...
 * LICENSE file in the root directory of this source tree.
 */
var aa=__webpack_require__(2460),e=__webpack_require__(4504),k=null,l=0,m=!0;function p(a,b){a=a.write(b);m=m&&a}
...
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4504);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
const BackgroundGlow=()=>{const{0:mousePosition,1:setMousePosition}=(0,react__WEBPACK_IMPORTED_MODULE_0__.useState)({x:0,y:0});const frame=(0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)();(0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(()=>{const reduceMotion=window.matchMedia('(prefers-reduced-motion: reduce)').matches;if(reduceMotion)return;const handleMouseMove=event=>{if(frame.current)cancelAnimationFrame(frame.current);frame.current=requestAnimationFrame(()=>{setMousePosition({x:event.clientX,y:event.clientY});});};window.addEventListener('mousemove',handleMouseMove,{passive:true});return()=>{window.removeEventListener('mousemove',handleMouseMove);if(frame.current)cancelAnimationFrame(frame.current);};},[]);return/*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div",{className:"pointer-events-none fixed inset-0 -z-10 transition duration-300",style:{background:`
          ...
          #060E17
        `,backgroundBlendMode:'screen'}});};/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (BackgroundGlow);
...
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4504);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _src_components_layout_BackgroundGlow__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(4411);
const wrapPageElement=({element})=>/*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement((react__WEBPACK_IMPORTED_MODULE_0___default().Fragment),null,/*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(_src_components_layout_BackgroundGlow__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .A,null),element);
...
exports.components={"component---src-pages-blog-index-tsx":()=>Promise.all(/* import() | component---src-pages-blog-index-tsx */[__webpack_require__.e(438), __webpack_require__.e(248)]).then(__webpack_require__.bind(__webpack_require__, 3514)),"component---src-pages-contact-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-contact-tsx */ 313).then(__webpack_require__.bind(__webpack_require__, 2511)),"component---src-pages-index-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-index-tsx */ 245).then(__webpack_require__.bind(__webpack_require__, 9208)),"component---src-pages-services-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-services-tsx */ 819).then(__webpack_require__.bind(__webpack_require__, 7755)),"component---src-pages-website-plans-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-website-plans-tsx */ 501).then(__webpack_require__.bind(__webpack_require__, 6464)),"component---src-templates-blog-post-tsx":()=>__webpack_require__.e(/* import() | component---src-templates-blog-post-tsx */ 455).then(__webpack_require__.bind(__webpack_require__, 8733))};
...
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(4504);
...
/* harmony export */ });
var plugins=[{name:'gatsby-plugin-image',plugin:__webpack_require__(70),options:{"plugins":[]}},{name:'gatsby-plugin-react-helmet',plugin:__webpack_require__(6560),options:{"plugins":[]}},{name:'default-site-plugin',plugin:__webpack_require__(5328),options:{"plugins":[]}},{name:'partytown',plugin:__webpack_require__(387),options:{"plugins":[]}}];/* global plugins */// During bootstrap, we write requires at top of this file which looks like:
// var plugins = [
//   {
...
/******/ 			return "" + ({"245":"component---src-pages-index-tsx","248":"component---src-pages-blog-index-tsx","313":"component---src-pages-contact-tsx","455":"component---src-templates-blog-post-tsx","501":"component---src-pages-website-plans-tsx","819":"component---src-pages-services-tsx"}[chunkId] || chunkId) + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
const React=__webpack_require__(4504);const path=__webpack_require__(2003);const{renderToStaticMarkup,renderToPipeableStream}=__webpack_require__(1552);const{ServerLocation,Router,isRedirect}=__webpack_require__(6462);const merge=__webpack_require__(4744);const{StaticQueryContext}=__webpack_require__(123);const fs=__webpack_require__(2243);const{WritableAsPromise}=__webpack_require__(5297);const{RouteAnnouncerProps}=__webpack_require__(6858);const{apiRunner,apiRunnerAsync}=__webpack_require__(9432);const asyncRequires=__webpack_require__(6877);const{version:gatsbyVersion}=__webpack_require__(5818);const{grabMatchParams}=__webpack_require__(5870);const{headHandlerForSSR}=__webpack_require__(988);const{SlicesResultsContext,SlicesContext,SlicesMapContext,SlicesPropsContext}=__webpack_require__(2024);const{ServerSliceRenderer}=__webpack_require__(8455);// we want to force posix-style joins, so Windows doesn't produce backslashes for urls
